...
makeVotingAppServer <- function(db_pool, cfg) {
  function(input, output, session) {
    ...
    # Track the last modification time of the to_be_voted_images_file
    last_modified_time <- shiny::reactiveVal(NULL)

    # Initialize with current file modification time
    shiny::observe({
      if (file.exists(cfg$to_be_voted_images_file)) {
        last_modified_time(file.info(cfg$to_be_voted_images_file)$mtime)
      }
    })

    # Every 5 seconds, check if the to_be_voted_images_file has been modified
    shiny::observe({
      ...
      current_mtime <- file.info(cfg$to_be_voted_images_file)$mtime

      # Check if this is the first time or if the file has been modified
      if (!is.null(last_modified_time()) && current_mtime > last_modified_time()) {
        cat("Detected change in to_be_voted_images_file\n")
        ...
        tryCatch({
          ...
          if (new_entries_count > 0) {
            ...
            cat(sprintf("Total annotations in DB after update: %s\n", total_images))
          }
        }, error = function(e) {
